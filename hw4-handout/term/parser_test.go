...
type acrossTermTestGeneratorFunction func() ([]string, []*Term)
...
// tests for share across terms
func acrossTermSharingTest0() ([]string, []*Term) {
	f := &Term{Typ: TermAtom, Literal: "f"}
	X := &Term{Typ: TermVariable, Literal: "X"}
	fX := &Term{Typ: TermCompound, Functor: f, Args: []*Term{X}}

	a := &Term{Typ: TermAtom, Literal: "a"}
	afX := &Term{Typ: TermCompound, Functor: a, Args: []*Term{fX}}

	b := &Term{Typ: TermAtom, Literal: "b"}
	bfX := &Term{Typ: TermCompound, Functor: b, Args: []*Term{fX}}

	return []string{"a( f(X) )", "b( f(X) )"}, []*Term{afX, bfX}
}


func TestAcrossTermSharing(t *testing.T) {
	for idx, testGenerator := range []acrossTermTestGeneratorFunction{
		acrossTermSharingTest0,
	} {
		func() {
			defer func() {
				if r := recover(); r != nil {
			...
			if areIsomorphic, err := checkAcrossTermIsomorphic(expected, actual); !areIsomorphic {
				t.Errorf("\nin test %d (\"%s\")%s", idx, input, err)
			}
		}()
	}
...
func checkAcrossTermIsomorphic(expected, actual []*Term) (bool, error) {
	matchTerms := make(map[*Term]*Term)
	for i, _ := range expected {
		if isomorphic, err := checkTermIsomorphic(expected[i], actual[i], matchTerms); !isomorphic {
			return isomorphic, err
		}
	}
	return true, nil
}
